...
def test_modrinth_url_type_override(project_env, run_puppy):
    """project_type changes the MR URL segment (mod -> /mod/)."""
    (project_env['project'] / 'puppy.yaml').write_text(
        yaml.dump({'project_type': 'mod', 'modrinth': {'slug': 'my-mod'}})
    )
    ...
    assert 'https://modrinth.com/mod/my-mod' in debug_file.read_text()
