...
def test_cf_description_is_html(project_env, run_puppy):
    ...
    (project_env["source"] / "description.md").write_text("# Hello\n\nA **bold** word.\n")
    run_puppy("push", "-n")

    content = (Path(tempfile.gettempdir()) / "puppy" / "neonglow" / "curseforge" / "description.html").read_text()
...
def test_pmc_description_is_bbcode(project_env, run_puppy):
    ...
    content = (Path(tempfile.gettempdir()) / "puppy" / "neonglow" / "planetminecraft" / "description.bbcode").read_text()
    assert "[h1]" in content
    assert "[b]" in content
    assert "**" not in content
...
def test_pmc_soft_wraps_preserved(project_env, run_puppy):
    (project_env["source"] / "puppy.yaml").write_text(yaml.dump({"version": "1.0"}))
    (project_env["source"] / "description.md").write_text("Word one\nword two on next line.\n")
    run_puppy("push", "-n")
...
def test_pmc_bbcode_renders_in_preview(project_env, run_puppy):
    (project_env["source"] / "puppy.yaml").write_text(yaml.dump({"version": "1.0"}))
    (project_env["source"] / "description.md").write_text("# Big Heading\n\nSome text.\n")
    run_puppy("push", "-n")
